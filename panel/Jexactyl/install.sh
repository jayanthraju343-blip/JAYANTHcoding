...
sed -i 's|MAIL_FROM_NAME=.*|MAIL_FROM_NAME="JAYANTH"|g' .env
...
sed -i '/APP_NAME=/d' .env && echo 'APP_NAME="JAYANTH"' >> .env && php artisan config:clear && php artisan cache:clear && php artisan view:clear && php artisan config:cache && systemctl restart pteroq && systemctl restart nginx
chown -R www-data:www-data /var/www/jexactyl/*
