#!/bin/bash
# ===========================================================
# JAYANTH HUB - OBSIDIAN NEXT GEN (v12.0 - JAYANTH Edition)
...
render_ui() {
    ...
    echo -e "${GOLD}  ╚██████╔╝██║  ██║   ██║   ██║  ██║██║ ╚████║   ██║   ██║  ██║    ██║  ██║╚██████╔╝██████╔╝${NC}"
    echo -e "${GOLD}   ╚═════╝ ╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═══╝   ╚═╝   ╚═╝  ╚═╝    ╚═╝  ╚═╝ ╚═════╝ ╚═════╝ ${NC}"
    echo -e "                  ${G}JAYANTH EDITION — OBSIDIAN NEXT GEN${NC}"
...
while true; do
    ...
    case $opt in
        1) bash <(curl -s https://raw.githubusercontent.com/jayanthraju343-blip/JAYANTH-Cloud/refs/heads/main/setup%20vm/menu.sh) ;;
        2) bash <(curl -s https://raw.githubusercontent.com/jayanthraju343-blip/JAYANTH-Cloud/refs/heads/main/panel/1.sh) ;;
        3) bash <(curl -s https://raw.githubusercontent.com/jayanthraju343-blip/JAYANTH-Cloud/refs/heads/main/wings/run.sh) ;;
        4) bash <(curl -s https://raw.githubusercontent.com/nobita329/ptero/refs/heads/main/ptero/tools/run.sh) ;;
        5) bash <(curl -s https://raw.githubusercontent.com/jayanthraju343-blip/JAYANTH-Cloud/refs/heads/main/thame/run.sh) ;;
        ...
        9) 
            ...
            bash <(curl -s https://raw.githubusercontent.com/jayanthraju343-blip/JAYANTH-Cloud/refs/heads/main/Extras/run.sh)
            echo -e " ${G}More features coming in future updates.${NC}"
            sleep 1.5
            ;;

        0|exit|quit)
            echo -e "\n ${B_RED}● DISCONNECTED${NC}  Goodbye, JAYANTH."
            exit 0 
            ;;
